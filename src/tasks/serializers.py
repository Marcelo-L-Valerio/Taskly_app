...
class TaskListSerializer(serializers.ModelSerializer):
    ...
    team = serializers.HyperlinkedRelatedField(queryset=Team.objects.all(), many=False, view_name='team-detail')
    ...
    class Meta:
        ...
        'created_on', 'created_by', 'team', 'tasks']
...
class TaskSerializer(serializers.ModelSerializer):
    ...
    def validate_task_list(self, value):
        ...
        if value not in user_profile.team.lists.all():
            raise serializers.ValidationError('Task list provided does not belong to the users team')
        return value
...
class AttachmentSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if task_list not in user_profile.team.lists.all():
            raise serializers.ValidationError({'task':'Task list provided does not belong to the users team'})
        return attrs
