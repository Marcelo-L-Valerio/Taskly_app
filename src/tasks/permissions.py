...
class IsAllowedToEditTaskListElseNone(permissions.BasePermission):
    '''Custom permission to tasklist viewset to only allow 
...
class IsAllowedToEditTaskElseNone(permissions.BasePermission):
    '''Custom permission to task viewset to only allow 
    members of a team access permission.'''

    def has_permission(self, request, view):

        if not request.user.is_anonymous:
            return request.user.profile.team != None
...
class IsAllowedToEditAttachmentElseNone(permissions.BasePermission):
    '''Custom permission to attachment viewset to only allow 
    members of a team access permission.'''
